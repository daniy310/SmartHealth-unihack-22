<footer class="d-flex flex-wrap justify-content-between align-items-center text-center p-4 bg-dark ">
      <p class="col-md-4 mb-0 text-muted">Created with ❤️ by Bucur Alexandru and Daniel Șelaru © 2022</p>

      <a href="index.php" class="col-md-4 d-flex align-items-center justify-content-center mb-3 mb-md-0 me-md-auto link-dark text-decoration-none">
            <img src="includes/images/logo.png" class="logo" alt="logo">
      </a>

      <ul class="nav col-md-4 justify-content-end">
            <li class="nav-item"><a href="index.php" class="nav-link px-2 text-muted">Home</a></li>
            <li class="nav-item"><a href="index.php#faq" class="nav-link px-2 text-muted">FAQs</a></li>
            <li class="nav-item"><a href="index.php#about" class="nav-link px-2 text-muted">About</a></li>
      </ul>
</footer>

</html>